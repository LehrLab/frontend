...
describe('validator.validatePasswordBoolean', () => {
  ...
  it('invalid passwords fail', () => {
    expect(validator.validatePasswordBoolean('abcdef1!')).toBe(false); // нет заглавной
    expect(validator.validatePasswordBoolean('ABCDEF1!')).toBe(false); // нет строчной
    expect(validator.validatePasswordBoolean('Abcdefgh!')).toBe(false); // нет цифры
    expect(validator.validatePasswordBoolean('Abcdef12')).toBe(false); // нет спецсимвола
    expect(validator.validatePasswordBoolean('A1!a')).toBe(false); // короткий
  });
});
...
describe('validator.validateEmail', () => {
  it('valid emails pass', () => {
    ...
    expect(validator.validateEmail('user.name+tag+sorting@example.co.uk')).toBe(true);
